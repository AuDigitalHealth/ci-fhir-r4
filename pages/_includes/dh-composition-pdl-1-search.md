...
1. **SHALL** support searching using the combination **`subject:identifier`** and **`category`** search parameter:
    ...
    Example:
    ~~~
    GET [base]/Composition?subject:identifier=http://ns.electronichealth.net.au/id/hi/ihi/1.0|8003608000228437&category=http://loinc.org|57828-6
    ~~~
    *Implementation Notes:* Fetches a bundle of all Composition resources with the category of "Prescription list" for the specified patient ([how to search by :identifier](http://hl7.org/fhir/R4/search.html#reference) and [how to search by token](http://hl7.org/fhir/search.html#token))
...
1. **SHALL** support searching using the combination **`subject:identifier`** and **`type`** search parameter:
    ...
    Example:
    ~~~
    GET [base]/Composition?subject:identifier=http://ns.electronichealth.net.au/id/hi/ihi/1.0|8003608000228437&type=http://loinc.org|57828-6
    ~~~
    *Implementation Notes:* Fetches a bundle of all Composition resources with the type of an 57828-6 (Prescription list) for the specified patient ([how to search by :identifier](http://hl7.org/fhir/R4/search.html#reference) and [how to search by token](http://hl7.org/fhir/search.html#token))
