...
1. **SHALL** support searching using the combination **`subject:identifier`** and **`category`** search parameter:
    ...
    Example:
    ~~~
    GET [base]/Composition?subject:identifier=http://ns.electronichealth.net.au/id/hi/ihi/1.0|8003608000228437&category=https://healthterminologies.gov.au/fhir/CodeSystem/nctis-data-components-1|100.16975
    ~~~
...
1. **SHALL** support searching using the combination **`subject:identifier`** and **`type`** search parameter:
    ...
    Example:
    ~~~
    GET [base]/Composition?subject:identifier=http://ns.electronichealth.net.au/id/hi/ihi/1.0|8003608000228437&type=https://healthterminologies.gov.au/fhir/CodeSystem/nctis-data-components-1|100.16696
    ~~~
    *Implementation Notes:* Fetches a bundle of all Composition resources with the type of an 100.16696 (Advance Care Directive Custodian Record) for the specified patient ([how to search by :identifier](http://hl7.org/fhir/R4/search.html#reference) and [how to search by token](http://hl7.org/fhir/search.html#token))
